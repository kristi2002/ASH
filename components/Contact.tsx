...
const inputClass =
  "w-full border-0 border-b border-line bg-transparent py-3 text-cream outline-none transition-colors duration-300 placeholder:text-cream-dim/40 focus:border-gold aria-[invalid=true]:border-[#c9714e]";
...
function SubmitButton() {
  ...
  return (
    <button
      ...
      disabled={pending}
      aria-busy={pending}
      className="group inline-flex items-center gap-3 border border-gold/60 px-8 py-4 font-display text-sm uppercase tracking-[0.22em] text-gold transition-colors duration-300 hover:bg-gold hover:text-charcoal disabled:cursor-not-allowed disabled:border-line disabled:text-cream-dim disabled:hover:bg-transparent"
    >
...
export default function Contact() {
  ...
  return (
    <section
      id="contatti"
      ref={root}
      className="relative overflow-hidden bg-charcoal px-6 py-24 md:px-12 md:py-32"
    >
      {/* Ambient glow + grain so the closing section isn't barren */}
        ...
        <div className="mt-14 grid gap-x-20 gap-y-16 lg:mt-24 lg:grid-cols-[0.9fr_1.1fr]">
          ...
          <div>
            {state.ok === true ? (
              <div
                role="status"
                className="flex min-h-[18rem] flex-col justify-center border-t border-line pt-10"
              >
              ...
              <form action={action} noValidate className="w-full">
                ...
                {state.ok === false && state.message && (
                  <p
                    role="alert"
                    className="mb-8 border-l-2 border-[#c9714e] pl-4 text-sm text-[#c9714e]"
                  >
                    {state.message}
                  </p>
                )}
                ...
                <div className="grid gap-x-10 gap-y-9 sm:grid-cols-2">
                  ...
                  <div data-field>
                    ...
                    {err.nome && (
                      <p id="nome-err" role="alert" className="mt-2 text-xs text-[#c9714e]">
                        {err.nome}
                      </p>
                    )}
                  </div>
                  ...
                  <div data-field>
                    ...
                    {err.email && (
                      <p id="email-err" role="alert" className="mt-2 text-xs text-[#c9714e]">
                        {err.email}
                      </p>
                    )}
                  </div>
                  ...
                  <div data-field>
                    ...
                    {err.telefono && (
                      <p id="telefono-err" role="alert" className="mt-2 text-xs text-[#c9714e]">
                        {err.telefono}
                      </p>
                    )}
                  </div>
                  ...
                  <div data-field>
                    ...
                    <div className="relative mt-3">
                      ...
                      >
                        <option value="" className="bg-charcoal text-cream-dim">
                          Seleziona…
                        </option>
                        {SERVICE_NAMES.map((s) => (
                    ...
                    {err.servizio && (
                      <p id="servizio-err" role="alert" className="mt-2 text-xs text-[#c9714e]">
                        {err.servizio}
                      </p>
                    )}
                  </div>
                  ...
                  <div data-field className="sm:col-span-2">
                    ...
                    {err.messaggio && (
                      <p id="messaggio-err" role="alert" className="mt-2 text-xs text-[#c9714e]">
                        {err.messaggio}
                      </p>
                    )}
                  </div>
